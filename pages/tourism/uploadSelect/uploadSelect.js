...
Page({
  ...
  onSpot: function (e) {
    ...
    status = status ? 0 : 1;
    if (id == this.data.spotId){
      id = 0;
    }

    if (this.data.spotTag != undefined) {
      let spotList = 'spotList[' + this.data.spotTag + '].status';
      this.setData({
        [spotList]: 0,
      })
    }
    let spotStatus = 'spotList[' + index + '].status';
    this.setData({
      [spotStatus]: status,
      spotTag: index,
      spotId: id,
